git clone https://github.com/ethereum/yellowpaper
git clone https://github.com/ethereum/solidity
git clone https://github.com/ethereum/cpp-ethereum
git clone https://github.com/ethereum/tests
git clone https://github.com/ethereum/eth-bloom
git clone https://github.com/ethereum/py-evm
git clone https://github.com/ethereum/go-ethereum
git clone https://github.com/ethereum/ethereumj
git clone https://github.com/ethereum/eth-tester
git clone https://github.com/ethereum/web3.py
git clone https://github.com/ethereum/homestead-guide
git clone https://github.com/ethereum/remix
git clone https://github.com/ethereum/web3.js
git clone https://github.com/ethereum/plasma
git clone https://github.com/ethereum/mist
git clone https://github.com/ethereum/cbc-casper
git clone https://github.com/ethereum/evmjit
git clone https://github.com/ethereum/vyper
git clone https://github.com/ethereum/solc-bin
git clone https://github.com/ethereum/solidity-test-bytecode
git clone https://github.com/ethereum/eth-abi
git clone https://github.com/ethereum/wiki
git clone https://github.com/ethereum/ethereum-org
#git clone https://github.com/ethereum/hunter-cache
git clone https://github.com/ethereum/remixd
git clone https://github.com/ethereum/EIPs
git clone https://github.com/ethereum/browser-solidity
git clone https://github.com/ethereum/meteor-dapp-wallet
git clone https://github.com/ethereum/swarm-dapps
git clone https://github.com/ethereum/py-trie
git clone https://github.com/ethereum/sharding
git clone https://github.com/ethereum/eth-utils
git clone https://github.com/ethereum/eth-keys
git clone https://github.com/ethereum/research
git clone https://github.com/ethereum/pyethereum
git clone https://github.com/ethereum/py_pairing
git clone https://github.com/ethereum/casper
git clone https://github.com/ethereum/solc-js
git clone https://github.com/ethereum/remix-live
git clone https://github.com/ethereum/ethereum-org-website
git clone https://github.com/ethereum/populus
git clone https://github.com/ethereum/pyethapp
git clone https://github.com/ethereum/py-geth
git clone https://github.com/ethereum/pydevp2p
git clone https://github.com/ethereum/ethash
git clone https://github.com/ethereum/emacs-solidity
git clone https://github.com/ethereum/pyrlp
git clone https://github.com/ethereum/eth-keyfile
git clone https://github.com/ethereum/ens-registrar-dapp
git clone https://github.com/ethereum/dopple
git clone https://github.com/ethereum/pm
git clone https://github.com/ethereum/moon-browser
git clone https://github.com/ethereum/pyeth_keys
git clone https://github.com/ethereum/ens
git clone https://github.com/ethereum/homebrew-ethereum
git clone https://github.com/ethereum/py-solc
git clone https://github.com/ethereum/ecp
git clone https://github.com/ethereum/dapp-bin
git clone https://github.com/ethereum/py_ecc
git clone https://github.com/ethereum/documentation
git clone https://github.com/ethereum/common
git clone https://github.com/ethereum/solidity-examples
git clone https://github.com/ethereum/ropsten
git clone https://github.com/ethereum/serpent
git clone https://github.com/ethereum/meteor-dapp-catalog
git clone https://github.com/ethereum/moon-lang
git clone https://github.com/ethereum/btcrelay
git clone https://github.com/ethereum/blockies
git clone https://github.com/ethereum/langlab
git clone https://github.com/ethereum/meteor-package-accounts
git clone https://github.com/ethereum/moon-tool
git clone https://github.com/ethereum/meteor-package-tools
git clone https://github.com/ethereum/rpc-tests
git clone https://github.com/ethereum/benchmarking
git clone https://github.com/ethereum/dapp-styles
git clone https://github.com/ethereum/meteor-package-elements
git clone https://github.com/ethereum/stake-voice
git clone https://github.com/ethereum/cpp-dependencies
#git clone https://github.com/ethereum/go-casper
git clone https://github.com/ethereum/webthree-umbrella
git clone https://github.com/ethereum/ethereum-client-binaries
git clone https://github.com/ethereum/ethendance
git clone https://github.com/ethereum/simplecasper
git clone https://github.com/ethereum/oyente
git clone https://github.com/ethereum/diary
git clone https://github.com/ethereum/meteor-dapp-whisper-chat-client
git clone https://github.com/ethereum/ethereum-console
git clone https://github.com/ethereum/libsnark
git clone https://github.com/ethereum/scavenger_hunt
git clone https://github.com/ethereum/statesweep
git clone https://github.com/ethereum/meteor-ethereum-networkinfo
git clone https://github.com/ethereum/meteor-package-blocks
git clone https://github.com/ethereum/interfaces
#git clone https://github.com/ethereum/evmc
git clone https://github.com/ethereum/webthree-helpers
git clone https://github.com/ethereum/webthree
git clone https://github.com/ethereum/libweb3core
git clone https://github.com/ethereum/libethereum
git clone https://github.com/ethereum/alethzero
git clone https://github.com/ethereum/mix
git clone https://github.com/ethereum/economic-modeling
git clone https://github.com/ethereum/libwhisper
git clone https://github.com/ethereum/ethereum-buildbot
git clone https://github.com/ethereum/test-tools
git clone https://github.com/ethereum/vim-solidity
#git clone https://github.com/ethereum/leveldb
#git clone https://github.com/ethereum/cpp-dependencies-win64
#git clone https://github.com/ethereum/cryptopp
git clone https://github.com/ethereum/ethereum-dockers
git clone https://github.com/ethereum/ethereum-ppa
#git clone https://github.com/ethereum/jenkins-docker
git clone https://github.com/ethereum/guide
git clone https://github.com/ethereum/bench
git clone https://github.com/ethereum/devp2p
git clone https://github.com/ethereum/ethbot
git clone https://github.com/ethereum/node-ethereum
git clone https://github.com/ethereum/ethereum-docs
git clone https://github.com/ethereum/gethkey
git clone https://github.com/ethereum/cpp-ethereum-cmake
git clone https://github.com/ethereum/devcommon
git clone https://github.com/ethereum/genesis_block_generator
git clone https://github.com/ethereum/frontier-guide
git clone https://github.com/ethereum/pyethsaletool
git clone https://github.com/ethereum/solidity.js
git clone https://github.com/ethereum/ethereum.org
git clone https://github.com/ethereum/system-testing
git clone https://github.com/ethereum/jeff
#git clone https://github.com/ethereum/gav
git clone https://github.com/ethereum/go-build
git clone https://github.com/ethereum/natspec.js
git clone https://github.com/ethereum/profiling
git clone https://github.com/ethereum/cpp-ethash
git clone https://github.com/ethereum/ethdev-site
git clone https://github.com/ethereum/www
git clone https://github.com/ethereum/serpent-go
git clone https://github.com/ethereum/foundation
git clone https://github.com/ethereum/tryethereum
git clone https://github.com/ethereum/mining
git clone https://github.com/ethereum/news
#git clone https://github.com/ethereum/branding

gource --output-custom-log yellowpaper.txt yellowpaper
gource --output-custom-log solidity.txt solidity
gource --output-custom-log cpp-ethereum.txt cpp-ethereum
gource --output-custom-log tests.txt tests
gource --output-custom-log eth-bloom.txt eth-bloom
gource --output-custom-log py-evm.txt py-evm
gource --output-custom-log go-ethereum.txt go-ethereum
gource --output-custom-log ethereumj.txt ethereumj
gource --output-custom-log eth-tester.txt eth-tester
gource --output-custom-log web3.py.txt web3.py
gource --output-custom-log homestead-guide.txt homestead-guide
gource --output-custom-log remix.txt remix
gource --output-custom-log web3.js.txt web3.js
gource --output-custom-log plasma.txt plasma
gource --output-custom-log mist.txt mist
gource --output-custom-log cbc-casper.txt cbc-casper
gource --output-custom-log evmjit.txt evmjit
gource --output-custom-log vyper.txt vyper
gource --output-custom-log solc-bin.txt solc-bin
gource --output-custom-log solidity-test-bytecode.txt solidity-test-bytecode
gource --output-custom-log eth-abi.txt eth-abi
gource --output-custom-log wiki.txt wiki
gource --output-custom-log ethereum-org.txt ethereum-org
#gource --output-custom-log hunter-cache.txt hunter-cache
gource --output-custom-log remixd.txt remixd
gource --output-custom-log EIPs.txt EIPs
gource --output-custom-log browser-solidity.txt browser-solidity
gource --output-custom-log meteor-dapp-wallet.txt meteor-dapp-wallet
gource --output-custom-log swarm-dapps.txt swarm-dapps
gource --output-custom-log py-trie.txt py-trie
gource --output-custom-log sharding.txt sharding
gource --output-custom-log eth-utils.txt eth-utils
gource --output-custom-log eth-keys.txt eth-keys
gource --output-custom-log research.txt research
gource --output-custom-log pyethereum.txt pyethereum
gource --output-custom-log py_pairing.txt py_pairing
gource --output-custom-log casper.txt casper
gource --output-custom-log solc-js.txt solc-js
gource --output-custom-log remix-live.txt remix-live
gource --output-custom-log ethereum-org-website.txt ethereum-org-website
gource --output-custom-log populus.txt populus
gource --output-custom-log pyethapp.txt pyethapp
gource --output-custom-log py-geth.txt py-geth
gource --output-custom-log pydevp2p.txt pydevp2p
gource --output-custom-log ethash.txt ethash
gource --output-custom-log emacs-solidity.txt emacs-solidity
gource --output-custom-log pyrlp.txt pyrlp
gource --output-custom-log eth-keyfile.txt eth-keyfile
gource --output-custom-log ens-registrar-dapp.txt ens-registrar-dapp
gource --output-custom-log dopple.txt dopple
gource --output-custom-log pm.txt pm
gource --output-custom-log moon-browser.txt moon-browser
gource --output-custom-log pyeth_keys.txt pyeth_keys
gource --output-custom-log ens.txt ens
gource --output-custom-log homebrew-ethereum.txt homebrew-ethereum
gource --output-custom-log py-solc.txt py-solc
gource --output-custom-log ecp.txt ecp
gource --output-custom-log dapp-bin.txt dapp-bin
gource --output-custom-log py_ecc.txt py_ecc
gource --output-custom-log documentation.txt documentation
gource --output-custom-log common.txt common
gource --output-custom-log solidity-examples.txt solidity-examples
gource --output-custom-log ropsten.txt ropsten
gource --output-custom-log serpent.txt serpent
gource --output-custom-log meteor-dapp-catalog.txt meteor-dapp-catalog
gource --output-custom-log moon-lang.txt moon-lang
gource --output-custom-log btcrelay.txt btcrelay
gource --output-custom-log blockies.txt blockies
gource --output-custom-log langlab.txt langlab
gource --output-custom-log meteor-package-accounts.txt meteor-package-accounts
gource --output-custom-log moon-tool.txt moon-tool
gource --output-custom-log meteor-package-tools.txt meteor-package-tools
gource --output-custom-log rpc-tests.txt rpc-tests
gource --output-custom-log benchmarking.txt benchmarking
gource --output-custom-log dapp-styles.txt dapp-styles
gource --output-custom-log meteor-package-elements.txt meteor-package-elements
gource --output-custom-log stake-voice.txt stake-voice
gource --output-custom-log cpp-dependencies.txt cpp-dependencies
#gource --output-custom-log go-casper.txt go-casper
gource --output-custom-log webthree-umbrella.txt webthree-umbrella
gource --output-custom-log ethereum-client-binaries.txt ethereum-client-binaries
gource --output-custom-log ethendance.txt ethendance
gource --output-custom-log simplecasper.txt simplecasper
gource --output-custom-log oyente.txt oyente
gource --output-custom-log diary.txt diary
gource --output-custom-log meteor-dapp-whisper-chat-client.txt meteor-dapp-whisper-chat-client
gource --output-custom-log ethereum-console.txt ethereum-console
gource --output-custom-log libsnark.txt libsnark
gource --output-custom-log scavenger_hunt.txt scavenger_hunt
gource --output-custom-log statesweep.txt statesweep
gource --output-custom-log meteor-ethereum-networkinfo.txt meteor-ethereum-networkinfo
gource --output-custom-log meteor-package-blocks.txt meteor-package-blocks
gource --output-custom-log interfaces.txt interfaces
#gource --output-custom-log evmc.txt evmc
gource --output-custom-log webthree-helpers.txt webthree-helpers
gource --output-custom-log webthree.txt webthree
gource --output-custom-log libweb3core.txt libweb3core
gource --output-custom-log libethereum.txt libethereum
gource --output-custom-log alethzero.txt alethzero
gource --output-custom-log mix.txt mix
gource --output-custom-log economic-modeling.txt economic-modeling
gource --output-custom-log libwhisper.txt libwhisper
gource --output-custom-log ethereum-buildbot.txt ethereum-buildbot
gource --output-custom-log test-tools.txt test-tools
gource --output-custom-log vim-solidity.txt vim-solidity
#gource --output-custom-log leveldb.txt leveldb
#gource --output-custom-log cpp-dependencies-win64.txt cpp-dependencies-win64
#gource --output-custom-log cryptopp.txt cryptopp
gource --output-custom-log ethereum-dockers.txt ethereum-dockers
gource --output-custom-log ethereum-ppa.txt ethereum-ppa
#gource --output-custom-log jenkins-docker.txt jenkins-docker
gource --output-custom-log guide.txt guide
gource --output-custom-log bench.txt bench
gource --output-custom-log devp2p.txt devp2p
gource --output-custom-log ethbot.txt ethbot
gource --output-custom-log node-ethereum.txt node-ethereum
gource --output-custom-log ethereum-docs.txt ethereum-docs
gource --output-custom-log gethkey.txt gethkey
gource --output-custom-log cpp-ethereum-cmake.txt cpp-ethereum-cmake
gource --output-custom-log devcommon.txt devcommon
gource --output-custom-log genesis_block_generator.txt genesis_block_generator
gource --output-custom-log frontier-guide.txt frontier-guide
gource --output-custom-log pyethsaletool.txt pyethsaletool
gource --output-custom-log solidity.js.txt solidity.js
gource --output-custom-log ethereum.org.txt ethereum.org
gource --output-custom-log system-testing.txt system-testing
gource --output-custom-log jeff.txt jeff
#gource --output-custom-log gav.txt gav
gource --output-custom-log go-build.txt go-build
gource --output-custom-log natspec.js.txt natspec.js
gource --output-custom-log profiling.txt profiling
gource --output-custom-log cpp-ethash.txt cpp-ethash
gource --output-custom-log ethdev-site.txt ethdev-site
gource --output-custom-log www.txt www
gource --output-custom-log serpent-go.txt serpent-go
gource --output-custom-log foundation.txt foundation
gource --output-custom-log tryethereum.txt tryethereum
gource --output-custom-log mining.txt mining
gource --output-custom-log news.txt news
#gource --output-custom-log branding.txt branding

sed -i -r "s#(.+)\|#\1|/yellowpaper#" yellowpaper.txt
sed -i -r "s#(.+)\|#\1|/solidity#" solidity.txt
sed -i -r "s#(.+)\|#\1|/cpp-ethereum#" cpp-ethereum.txt
sed -i -r "s#(.+)\|#\1|/tests#" tests.txt
sed -i -r "s#(.+)\|#\1|/eth-bloom#" eth-bloom.txt
sed -i -r "s#(.+)\|#\1|/py-evm#" py-evm.txt
sed -i -r "s#(.+)\|#\1|/go-ethereum#" go-ethereum.txt
sed -i -r "s#(.+)\|#\1|/ethereumj#" ethereumj.txt
sed -i -r "s#(.+)\|#\1|/eth-tester#" eth-tester.txt
sed -i -r "s#(.+)\|#\1|/web3.py#" web3.py.txt
sed -i -r "s#(.+)\|#\1|/homestead-guide#" homestead-guide.txt
sed -i -r "s#(.+)\|#\1|/remix#" remix.txt
sed -i -r "s#(.+)\|#\1|/web3.js#" web3.js.txt
sed -i -r "s#(.+)\|#\1|/plasma#" plasma.txt
sed -i -r "s#(.+)\|#\1|/mist#" mist.txt
sed -i -r "s#(.+)\|#\1|/cbc-casper#" cbc-casper.txt
sed -i -r "s#(.+)\|#\1|/evmjit#" evmjit.txt
sed -i -r "s#(.+)\|#\1|/vyper#" vyper.txt
sed -i -r "s#(.+)\|#\1|/solc-bin#" solc-bin.txt
sed -i -r "s#(.+)\|#\1|/solidity-test-bytecode#" solidity-test-bytecode.txt
sed -i -r "s#(.+)\|#\1|/eth-abi#" eth-abi.txt
sed -i -r "s#(.+)\|#\1|/wiki#" wiki.txt
sed -i -r "s#(.+)\|#\1|/ethereum-org#" ethereum-org.txt
#sed -i -r "s#(.+)\|#\1|/hunter-cache#" hunter-cache.txt
sed -i -r "s#(.+)\|#\1|/remixd#" remixd.txt
sed -i -r "s#(.+)\|#\1|/EIPs#" EIPs.txt
sed -i -r "s#(.+)\|#\1|/browser-solidity#" browser-solidity.txt
sed -i -r "s#(.+)\|#\1|/meteor-dapp-wallet#" meteor-dapp-wallet.txt
sed -i -r "s#(.+)\|#\1|/swarm-dapps#" swarm-dapps.txt
sed -i -r "s#(.+)\|#\1|/py-trie#" py-trie.txt
sed -i -r "s#(.+)\|#\1|/sharding#" sharding.txt
sed -i -r "s#(.+)\|#\1|/eth-utils#" eth-utils.txt
sed -i -r "s#(.+)\|#\1|/eth-keys#" eth-keys.txt
sed -i -r "s#(.+)\|#\1|/research#" research.txt
sed -i -r "s#(.+)\|#\1|/pyethereum#" pyethereum.txt
sed -i -r "s#(.+)\|#\1|/py_pairing#" py_pairing.txt
sed -i -r "s#(.+)\|#\1|/casper#" casper.txt
sed -i -r "s#(.+)\|#\1|/solc-js#" solc-js.txt
sed -i -r "s#(.+)\|#\1|/remix-live#" remix-live.txt
sed -i -r "s#(.+)\|#\1|/ethereum-org-website#" ethereum-org-website.txt
sed -i -r "s#(.+)\|#\1|/populus#" populus.txt
sed -i -r "s#(.+)\|#\1|/pyethapp#" pyethapp.txt
sed -i -r "s#(.+)\|#\1|/py-geth#" py-geth.txt
sed -i -r "s#(.+)\|#\1|/pydevp2p#" pydevp2p.txt
sed -i -r "s#(.+)\|#\1|/ethash#" ethash.txt
sed -i -r "s#(.+)\|#\1|/emacs-solidity#" emacs-solidity.txt
sed -i -r "s#(.+)\|#\1|/pyrlp#" pyrlp.txt
sed -i -r "s#(.+)\|#\1|/eth-keyfile#" eth-keyfile.txt
sed -i -r "s#(.+)\|#\1|/ens-registrar-dapp#" ens-registrar-dapp.txt
sed -i -r "s#(.+)\|#\1|/dopple#" dopple.txt
sed -i -r "s#(.+)\|#\1|/pm#" pm.txt
sed -i -r "s#(.+)\|#\1|/moon-browser#" moon-browser.txt
sed -i -r "s#(.+)\|#\1|/pyeth_keys#" pyeth_keys.txt
sed -i -r "s#(.+)\|#\1|/ens#" ens.txt
sed -i -r "s#(.+)\|#\1|/homebrew-ethereum#" homebrew-ethereum.txt
sed -i -r "s#(.+)\|#\1|/py-solc#" py-solc.txt
sed -i -r "s#(.+)\|#\1|/ecp#" ecp.txt
sed -i -r "s#(.+)\|#\1|/dapp-bin#" dapp-bin.txt
sed -i -r "s#(.+)\|#\1|/py_ecc#" py_ecc.txt
sed -i -r "s#(.+)\|#\1|/documentation#" documentation.txt
sed -i -r "s#(.+)\|#\1|/common#" common.txt
sed -i -r "s#(.+)\|#\1|/solidity-examples#" solidity-examples.txt
sed -i -r "s#(.+)\|#\1|/ropsten#" ropsten.txt
sed -i -r "s#(.+)\|#\1|/serpent#" serpent.txt
sed -i -r "s#(.+)\|#\1|/meteor-dapp-catalog#" meteor-dapp-catalog.txt
sed -i -r "s#(.+)\|#\1|/moon-lang#" moon-lang.txt
sed -i -r "s#(.+)\|#\1|/btcrelay#" btcrelay.txt
sed -i -r "s#(.+)\|#\1|/blockies#" blockies.txt
sed -i -r "s#(.+)\|#\1|/langlab#" langlab.txt
sed -i -r "s#(.+)\|#\1|/meteor-package-accounts#" meteor-package-accounts.txt
sed -i -r "s#(.+)\|#\1|/moon-tool#" moon-tool.txt
sed -i -r "s#(.+)\|#\1|/meteor-package-tools#" meteor-package-tools.txt
sed -i -r "s#(.+)\|#\1|/rpc-tests#" rpc-tests.txt
sed -i -r "s#(.+)\|#\1|/benchmarking#" benchmarking.txt
sed -i -r "s#(.+)\|#\1|/dapp-styles#" dapp-styles.txt
sed -i -r "s#(.+)\|#\1|/meteor-package-elements#" meteor-package-elements.txt
sed -i -r "s#(.+)\|#\1|/stake-voice#" stake-voice.txt
sed -i -r "s#(.+)\|#\1|/cpp-dependencies#" cpp-dependencies.txt
#sed -i -r "s#(.+)\|#\1|/go-casper#" go-casper.txt
sed -i -r "s#(.+)\|#\1|/webthree-umbrella#" webthree-umbrella.txt
sed -i -r "s#(.+)\|#\1|/ethereum-client-binaries#" ethereum-client-binaries.txt
sed -i -r "s#(.+)\|#\1|/ethendance#" ethendance.txt
sed -i -r "s#(.+)\|#\1|/simplecasper#" simplecasper.txt
sed -i -r "s#(.+)\|#\1|/oyente#" oyente.txt
sed -i -r "s#(.+)\|#\1|/diary#" diary.txt
sed -i -r "s#(.+)\|#\1|/meteor-dapp-whisper-chat-client#" meteor-dapp-whisper-chat-client.txt
sed -i -r "s#(.+)\|#\1|/ethereum-console#" ethereum-console.txt
sed -i -r "s#(.+)\|#\1|/libsnark#" libsnark.txt
sed -i -r "s#(.+)\|#\1|/scavenger_hunt#" scavenger_hunt.txt
sed -i -r "s#(.+)\|#\1|/statesweep#" statesweep.txt
sed -i -r "s#(.+)\|#\1|/meteor-ethereum-networkinfo#" meteor-ethereum-networkinfo.txt
sed -i -r "s#(.+)\|#\1|/meteor-package-blocks#" meteor-package-blocks.txt
sed -i -r "s#(.+)\|#\1|/interfaces#" interfaces.txt
#sed -i -r "s#(.+)\|#\1|/evmc#" evmc.txt
sed -i -r "s#(.+)\|#\1|/webthree-helpers#" webthree-helpers.txt
sed -i -r "s#(.+)\|#\1|/webthree#" webthree.txt
sed -i -r "s#(.+)\|#\1|/libweb3core#" libweb3core.txt
sed -i -r "s#(.+)\|#\1|/libethereum#" libethereum.txt
sed -i -r "s#(.+)\|#\1|/alethzero#" alethzero.txt
sed -i -r "s#(.+)\|#\1|/mix#" mix.txt
sed -i -r "s#(.+)\|#\1|/economic-modeling#" economic-modeling.txt
sed -i -r "s#(.+)\|#\1|/libwhisper#" libwhisper.txt
sed -i -r "s#(.+)\|#\1|/ethereum-buildbot#" ethereum-buildbot.txt
sed -i -r "s#(.+)\|#\1|/test-tools#" test-tools.txt
sed -i -r "s#(.+)\|#\1|/vim-solidity#" vim-solidity.txt
#sed -i -r "s#(.+)\|#\1|/leveldb#" leveldb.txt
#sed -i -r "s#(.+)\|#\1|/cpp-dependencies-win64#" cpp-dependencies-win64.txt
#sed -i -r "s#(.+)\|#\1|/cryptopp#" cryptopp.txt
sed -i -r "s#(.+)\|#\1|/ethereum-dockers#" ethereum-dockers.txt
sed -i -r "s#(.+)\|#\1|/ethereum-ppa#" ethereum-ppa.txt
#sed -i -r "s#(.+)\|#\1|/jenkins-docker#" jenkins-docker.txt
sed -i -r "s#(.+)\|#\1|/guide#" guide.txt
sed -i -r "s#(.+)\|#\1|/bench#" bench.txt
sed -i -r "s#(.+)\|#\1|/devp2p#" devp2p.txt
sed -i -r "s#(.+)\|#\1|/ethbot#" ethbot.txt
sed -i -r "s#(.+)\|#\1|/node-ethereum#" node-ethereum.txt
sed -i -r "s#(.+)\|#\1|/ethereum-docs#" ethereum-docs.txt
sed -i -r "s#(.+)\|#\1|/gethkey#" gethkey.txt
sed -i -r "s#(.+)\|#\1|/cpp-ethereum-cmake#" cpp-ethereum-cmake.txt
sed -i -r "s#(.+)\|#\1|/devcommon#" devcommon.txt
sed -i -r "s#(.+)\|#\1|/genesis_block_generator#" genesis_block_generator.txt
sed -i -r "s#(.+)\|#\1|/frontier-guide#" frontier-guide.txt
sed -i -r "s#(.+)\|#\1|/pyethsaletool#" pyethsaletool.txt
sed -i -r "s#(.+)\|#\1|/solidity.js#" solidity.js.txt
sed -i -r "s#(.+)\|#\1|/ethereum.org#" ethereum.org.txt
sed -i -r "s#(.+)\|#\1|/system-testing#" system-testing.txt
sed -i -r "s#(.+)\|#\1|/jeff#" jeff.txt
#sed -i -r "s#(.+)\|#\1|/gav#" gav.txt
sed -i -r "s#(.+)\|#\1|/go-build#" go-build.txt
sed -i -r "s#(.+)\|#\1|/natspec.js#" natspec.js.txt
sed -i -r "s#(.+)\|#\1|/profiling#" profiling.txt
sed -i -r "s#(.+)\|#\1|/cpp-ethash#" cpp-ethash.txt
sed -i -r "s#(.+)\|#\1|/ethdev-site#" ethdev-site.txt
sed -i -r "s#(.+)\|#\1|/www#" www.txt
sed -i -r "s#(.+)\|#\1|/serpent-go#" serpent-go.txt
sed -i -r "s#(.+)\|#\1|/foundation#" foundation.txt
sed -i -r "s#(.+)\|#\1|/tryethereum#" tryethereum.txt
sed -i -r "s#(.+)\|#\1|/mining#" mining.txt
sed -i -r "s#(.+)\|#\1|/news#" news.txt
#sed -i -r "s#(.+)\|#\1|/branding#" branding.txt

cat yellowpaper.txt solidity.txt cpp-ethereum.txt tests.txt eth-bloom.txt py-evm.txt go-ethereum.txt ethereumj.txt eth-tester.txt web3.py.txt homestead-guide.txt remix.txt web3.js.txt plasma.txt mist.txt cbc-casper.txt evmjit.txt vyper.txt solc-bin.txt solidity-test-bytecode.txt eth-abi.txt wiki.txt ethereum-org.txt hunter-cache.txt remixd.txt EIPs.txt browser-solidity.txt meteor-dapp-wallet.txt swarm-dapps.txt py-trie.txt sharding.txt eth-utils.txt eth-keys.txt research.txt pyethereum.txt py_pairing.txt casper.txt solc-js.txt remix-live.txt ethereum-org-website.txt populus.txt pyethapp.txt py-geth.txt pydevp2p.txt ethash.txt emacs-solidity.txt pyrlp.txt eth-keyfile.txt ens-registrar-dapp.txt dopple.txt pm.txt moon-browser.txt pyeth_keys.txt ens.txt homebrew-ethereum.txt py-solc.txt ecp.txt dapp-bin.txt py_ecc.txt documentation.txt common.txt solidity-examples.txt ropsten.txt serpent.txt meteor-dapp-catalog.txt moon-lang.txt btcrelay.txt blockies.txt langlab.txt meteor-package-accounts.txt moon-tool.txt meteor-package-tools.txt rpc-tests.txt benchmarking.txt dapp-styles.txt meteor-package-elements.txt stake-voice.txt cpp-dependencies.txt go-casper.txt webthree-umbrella.txt ethereum-client-binaries.txt ethendance.txt simplecasper.txt oyente.txt diary.txt meteor-dapp-whisper-chat-client.txt ethereum-console.txt libsnark.txt scavenger_hunt.txt statesweep.txt meteor-ethereum-networkinfo.txt meteor-package-blocks.txt interfaces.txt evmc.txt webthree-helpers.txt webthree.txt libweb3core.txt libethereum.txt alethzero.txt mix.txt economic-modeling.txt libwhisper.txt ethereum-buildbot.txt test-tools.txt vim-solidity.txt leveldb.txt cpp-dependencies-win64.txt cryptopp.txt ethereum-dockers.txt ethereum-ppa.txt jenkins-docker.txt guide.txt bench.txt devp2p.txt ethbot.txt node-ethereum.txt ethereum-docs.txt gethkey.txt cpp-ethereum-cmake.txt devcommon.txt genesis_block_generator.txt frontier-guide.txt pyethsaletool.txt solidity.js.txt ethereum.org.txt system-testing.txt jeff.txt gav.txt go-build.txt natspec.js.txt profiling.txt cpp-ethash.txt ethdev-site.txt www.txt serpent-go.txt foundation.txt tryethereum.txt mining.txt news.txt branding.txt | sort -n > ethereum-project.txt

cd yellowpaper
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd solidity
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd cpp-ethereum
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd tests
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd eth-bloom
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd py-evm
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd go-ethereum
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd ethereumj
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd eth-tester
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd web3.py
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd homestead-guide
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd remix
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd web3.js
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd plasma
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd mist
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd cbc-casper
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd evmjit
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd vyper
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd solc-bin
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd solidity-test-bytecode
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd eth-abi
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd wiki
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd ethereum-org
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
#cd hunter-cache
#perl ../../lib/avatar.pl
#cp -rf .git/avatar ../
#cd ..
cd remixd
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd EIPs
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd browser-solidity
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd meteor-dapp-wallet
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd swarm-dapps
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd py-trie
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd sharding
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd eth-utils
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd eth-keys
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd research
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd pyethereum
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd py_pairing
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd casper
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd solc-js
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd remix-live
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd ethereum-org-website
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd populus
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd pyethapp
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd py-geth
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd pydevp2p
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd ethash
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd emacs-solidity
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd pyrlp
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd eth-keyfile
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd ens-registrar-dapp
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd dopple
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd pm
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd moon-browser
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd pyeth_keys
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd ens
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd homebrew-ethereum
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd py-solc
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd ecp
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd dapp-bin
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd py_ecc
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd documentation
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd common
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd solidity-examples
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd ropsten
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd serpent
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd meteor-dapp-catalog
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd moon-lang
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd btcrelay
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd blockies
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd langlab
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd meteor-package-accounts
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd moon-tool
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd meteor-package-tools
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd rpc-tests
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd benchmarking
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd dapp-styles
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd meteor-package-elements
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd stake-voice
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd cpp-dependencies
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
#cd go-casper
#perl ../../lib/avatar.pl
#cp -rf .git/avatar ../
#cd ..
cd webthree-umbrella
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd ethereum-client-binaries
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd ethendance
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd simplecasper
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd oyente
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd diary
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd meteor-dapp-whisper-chat-client
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd ethereum-console
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd libsnark
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd scavenger_hunt
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd statesweep
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd meteor-ethereum-networkinfo
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd meteor-package-blocks
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd interfaces
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
#cd evmc
#perl ../../lib/avatar.pl
#cp -rf .git/avatar ../
#cd ..
cd webthree-helpers
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd webthree
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd libweb3core
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd libethereum
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd alethzero
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd mix
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd economic-modeling
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd libwhisper
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd ethereum-buildbot
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd test-tools
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd vim-solidity
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
#cd leveldb
#perl ../../lib/avatar.pl
#cp -rf .git/avatar ../
#cd ..
#cd cpp-dependencies-win64
#perl ../../lib/avatar.pl
#cp -rf .git/avatar ../
#cd ..
#cd cryptopp
#perl ../../lib/avatar.pl
#cp -rf .git/avatar ../
#cd ..
cd ethereum-dockers
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd ethereum-ppa
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
#cd jenkins-docker
#perl ../../lib/avatar.pl
#cp -rf .git/avatar ../
#cd ..
cd guide
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd bench
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd devp2p
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd ethbot
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd node-ethereum
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd ethereum-docs
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd gethkey
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd cpp-ethereum-cmake
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd devcommon
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd genesis_block_generator
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd frontier-guide
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd pyethsaletool
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd solidity.js
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd ethereum.org
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd system-testing
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd jeff
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
#cd gav
#perl ../../lib/avatar.pl
#cp -rf .git/avatar ../
#cd ..
cd go-build
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd natspec.js
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd profiling
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd cpp-ethash
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd ethdev-site
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd www
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd serpent-go
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd foundation
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd tryethereum
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd mining
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
cd news
perl ../../lib/avatar.pl
cp -rf .git/avatar ../
cd ..
#cd branding
#perl ../../lib/avatar.pl
#cp -rf .git/avatar ../
#cd ..
