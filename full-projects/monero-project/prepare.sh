...
git clone https://github.com/monero-project/monero-forum
git clone https://github.com/monero-project/unbound
...
gource --output-custom-log monero-forum.txt monero-forum
gource --output-custom-log unbound.txt unbound
...
sed -i -r "s#(.+)\|#\1|/monero-forum#" monero-forum.txt
sed -i -r "s#(.+)\|#\1|/unbound#" unbound.txt
sed -i -r "s#(.+)\|#\1|/xmr-seeder#" xmr-seeder.txt
sed -i -r "s#(.+)\|#\1|/urs#" urs.txt
sed -i -r "s#(.+)\|#\1|/mininero#" mininero.txt

cat monero.txt monero-gui.txt kovri.txt kovri-site.txt meta.txt monero-site.txt sekura.txt research-lab.txt kovri-docs.txt supercop.txt miniupnpc.txt monero-forum.txt unbound.txt xmr-seeder.txt urs.txt mininero.txt | sort -n > monero-project.txt
