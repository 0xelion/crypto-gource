...
git clone https://github.com/EOSIO/logchain
...
gource --output-custom-log logchain.txt logchain
...
sed -i -r "s#(.+)\|#\1|/logchain#" logchain.txt
...
cat Documentation.txt eos.txt genesis.txt appbase.txt eosjs.txt eosjs-keygen.txt eosjs-api.txt eosjs-json.txt eosjs-ecc.txt eosjs-fcbuffer.txt eos-wallet-app.txt chainbase.txt logchain.txt eosjs-ui.txt eos-token-distribution.txt eosjs-secp256k1.txt | sort -n > eos-project.txt
